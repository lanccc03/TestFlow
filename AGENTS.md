...
TestFlow 是 pnpm workspace，包含 Vite Web、Electron 桌面壳和 FastAPI 后端。修改时先遵守本文件，再匹配目标文件既有风格；不做无关重构。
...
- TypeScript、Python 和 Electron 分别遵循目标目录既有风格；Python 遵守 `backend/pyproject.toml` 的 Ruff 规则。
- Web 应用层通常用单引号且不写分号；shadcn 生成文件可能用双引号；Electron 使用双引号和分号。
- Commit message 保持简短直接。

## Web

- 这是桌面端应用，不需要验证移动端布局。使用 Tailwind CSS v4 和 shadcn/ui；先查 `apps/web/src/components/ui/`，再决定是否新增组件。
...
- 优先使用语义化 Tailwind token、`cn()` 和既有布局约定。
- 业务代码按 feature-first 放在 `apps/web/src/features/<domain>/`；共享配置放 `apps/web/src/app/config.ts`；全局布局放 `apps/web/src/components/layout/`；`App.tsx` 保持为初始化和路由入口。

## Backend

- 分层遵循 `api/routes -> modules/*/service -> modules/*/{repository, adapters} -> db/integrations`。
- Route 只处理 HTTP/WebSocket 边界；业务编排放 service；领域数据读写放 repository；外部系统适配器放 `app.integrations.*`。
...
- Web 测试位于 `apps/web/src` 的 `*.test.ts(x)`；修改路由行为、API client、组件交互或非平凡 UI 状态时更新 Vitest。
- 测试断言行为和语义，优先用 role、label、文本、URL 和 ARIA；不要读取 `src/index.css` 断言业务 class。
- 可见 Web UI 变更需运行 `pnpm check:web`、`pnpm --filter @testflow/web test`。

## Agent

- 回答本仓库中关于 library、framework、SDK、API、CLI 或 cloud service 的问题时，先用 Context7 MCP 获取当前文档。
- 一般重构、业务逻辑调试、代码审查或从零编写脚本时不需要 Context7。
