...
后端采用模块化分层架构。日常开发必须遵守 `docs/backend-architecture-constraints.md`；更完整的设计背景见 `docs/superpowers/specs/2026-06-02-backend-architecture-design.md`。

核心原则：`api/routes -> modules/*/service -> repository/storage/integrations`。新增或修改后端业务代码时，优先使用 `app.core.*`、`app.modules.*`、`app.integrations.*` 等规范路径，不要向 `backend/app/` 新增顶层业务模块。
